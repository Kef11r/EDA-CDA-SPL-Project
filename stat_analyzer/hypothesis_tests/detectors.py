...
def detect_type(series: pd.Series) -> str:
    """Detects if the series is numeric or categorical."""
...
def _ensure_category_first(df: pd.DataFrame, col1: str,
    ...
    """Reorders column names so the categorical one comes first."""
...
def suggest_tests(df: pd.DataFrame, col1: str,
                  col2: str,) -> list[str]:
    """Suggests applicable statistical tests based on data types."""
    t1 = detect_type(df[col1])
    t2 = detect_type(df[col2])
    # numeric vs numeric
